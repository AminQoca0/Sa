const fetch = require('node-fetch'),
    ...
    admins = ["838330071411458068", "838330806845177856"],
    config = {
        "838330824914632724": 75,
        "838330837962457109": 50,
        "838330852042604564": 20,
        "838330865930469388": 10,
        "838325530255163402": 5
    }
...
client.on('message', async (message) => {
    ...
    if (command === "tfollow") {
        let bots = client.channels.cache.find(c => c.name === "bots").id
        if(message.channel.id !== client.channels.cache.find(c => c.name === "bots").id) return message.channel.send("Please use it on Channel <#" + bots +"> !")
        let twitchID = "";
        ...
        if (admins.includes(message.author.id)) {
            ...
            if (message.channel.id === client.channels.cache.find(c => c.name === "genel").id) return message.delete();
...
client.on("guildMemberAdd", async (member) => {
    ...
    for (let i = 0; i < channels.length; i++) {
        ...
        if (channel) channel.send(`${member}, **Check out this channel !**`)
    }
})
